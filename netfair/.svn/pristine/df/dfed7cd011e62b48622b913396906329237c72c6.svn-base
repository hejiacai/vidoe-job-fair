...
class controller_index extends components_cbasepage {
	...
	public function pageindex($inPath) {
        ...
        if ($serach_time>(time()-2) && $in_data['content']){
            $in_data['list'] = array();
            $in_data['serach_warn'] = '搜索过于频繁,请2秒后搜索';
		...
		if($this->isLogin() && $this->_usertype == 'p' ) {
            $in_data['is_person_login'] = 1;
		}else{
            $in_data['is_person_login'] = 0;
        }
		...
		if (empty($in_data['content'])){
            ...
            if ($cache_list['interview_in']||$cache_list['signup_in']||$cache_list['go_on']||$cache_list['over']){
                ...
                $in_data['list'] = $list;
                $cache->set('pc_netfair_list',serialize($list),'300');
            }
