<?php /* Smarty version Smarty-3.0.7, created on 2020-03-24 14:13:21
         compiled from "app\templates\huibo_chat/chat.html" */ ?>
<?php /*%%SmartyHeaderCode:72925e79a50112b125-97000493%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '77043c5c6b20665725bf7a46719c21029459c68d' => 
    array (
      0 => 'app\\templates\\huibo_chat/chat.html',
      1 => 1584689965,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '72925e79a50112b125-97000493',
  'function' => 
  array (
  ),
...
?><?php include_once ('app/controller/huibo_chat.php');?>
...
/*    .liaoyiliao_content{padding:10px; text-align:left;}
	...
	.liaoyiliao iframe{ height: 350px!important;}
	.buyPoints iframe{ height: 350px!important;}
</style>
...
/chat/Rebind/";
        ...
        bindDialog.show();
            ...
            dialog_msg = new Dialog({
                ...
                isAjax: false,
                isOverflow : false,
                content: '<?php echo smarty_function_get_url(array('rule'=>"/index/ShowHRManagerMsg/",'domain'=>"company"),$_smarty_tpl);?>
'
            });
    ...
    $("body").on("click",".chatOneChat",function(e){
        ...
        psot('<?php echo smarty_function_get_url(array('rule'=>"/resume/CheckCompanyLetter/",'domain'=>"company"),$_smarty_tpl);?>
...
/download/CheckBalanceV2/";
				...
				if(need_downlaod == 1 || true){
					var run1=_this.running();
						psot(checkDownloadUrl,data,function(result){
							run1.close()
							...
							if(result.status == '200'){
								...
								if(result.status == '401' || result.status == '402'){
										...
										dialog_msg_liao.setContent("<?php echo smarty_function_get_url(array('rule'=>'/resume/resumeinfomsghtml','domain'=>'company'),$_smarty_tpl);?>
").show();
								}
							}
...
/chat/checkChatHistoryV2?resume_id=" + resume_id;
			...
			psot(chat_history_url, {}, function (re) {
				run2.close()
//                re = JSON.parse(re);
                if (!re.status) {
                    ...
                    haveChatObj.on("click", ".goChat", function () {
                        haveChatDialog.hide();
                        var hr_person_url = "<?php echo smarty_function_get_url(array('rule'=>'/download/GetHrPersonChatHistory','domain'=>'company'),$_smarty_tpl);?>
";
                        psot(hr_person_url, {resume_id:resume_id}, function(re){
...
/chat/searchJob";
            ...
            psot(searchUrl,{station:station},function(result){ //获取职位
				$run.close();
                _this.show();
...
/img/company/cjl.jpg" /></dt>'
                            +'<dd>'
                                   ...
                                   +'<span>￥'+price+'</span></p>'
                                    +'<p><a href="<?php echo smarty_function_get_url(array('rule'=>"/pay/",'domain'=>"company"),$_smarty_tpl);?>
" target="_blank" class="btn1 btnsF14 close">立即充值</a><a href="<?php echo smarty_function_get_url(array('rule'=>"/index/memberdetail/",'domain'=>"company"),$_smarty_tpl);?>
...
");
    ...
    $(function () {
        psot('<?php echo smarty_function_get_url(array('rule'=>"/chat/GetChatCookie/",'domain'=>"company"),$_smarty_tpl);?>
?account_id='+chatGetCookie("accountid"),{},function (e) {
            chatLoginStatus = e.status;
...
function chatAreaLimit(){
    ...
    function psot(url,data,function_success) {
        $.ajax({
            url: url,
            type: 'post',
            data: data,
